...
/* eslint-disable indent */
    ...
    export default function (view, params) {
        ...
        events.on(syncPlayManager, 'enabled', (event, enabled) => {
            if (enabled) {
                // SyncPlay enabled
            } else {
                const syncPlayIcon = view.querySelector('#syncPlayIcon');
        ...
        events.on(syncPlayManager, 'notify-osd', (event, action) => {
            showIcon(action);
        });

        events.on(syncPlayManager, 'group-state-update', (event, state, reason) => {
